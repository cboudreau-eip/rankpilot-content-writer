...
export default function ArticleEditor() {
  ...
  const [imageSuggestions, setImageSuggestions] = useState<Array<{ afterHeading: string; prompt: string; altText: string }>>([]);
  const [generatingImageIdx, setGeneratingImageIdx] = useState<number | null>(null);

  const suggestImagesMutation = trpc.articleImages.suggestPlacements.useMutation({
    onSuccess: (data) => {
      setImageSuggestions(data.suggestions || []);
      setShowImagePanel(true);
      const count = data.suggestions?.length || 0;
      if (count === 0) {
        toast.info("No image placement suggestions — the article may already have images or be too short");
      } else {
        toast.success(`Found ${count} suggested image placement${count > 1 ? "s" : ""}`);
      }
    },
    onError: (err: any) => {
      toast.error(err.message || "Failed to suggest image placements");
    },
  });

  const generateSingleImageMutation = trpc.articleImages.generate.useMutation({
    onError: (err: any) => {
  ...
  const regenerateImageMutation = trpc.articleImages.regenerate.useMutation({
    onSuccess: () => {
  ...
  const removeImageMutation = trpc.articleImages.remove.useMutation({
    onSuccess: () => {
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-52">
              ...
              <DropdownMenuItem
                onClick={() => {
                  if (showImagePanel) { setShowImagePanel(false); return; }
                  setShowImagePanel(true);
                  suggestImagesMutation.mutate({ articleId, projectId: (article as any)?.projectId });
                }}
                disabled={suggestImagesMutation.isPending}
                className={showImagePanel ? "bg-purple-50 text-purple-700" : ""}
              >
                {suggestImagesMutation.isPending ? (
                  <Loader2 className="w-4 h-4 mr-2 animate-spin" />
                ) : (
                  <ImagePlus className="w-4 h-4 mr-2 text-purple-600" />
                )}
                Add Images
              </DropdownMenuItem>
              <DropdownMenuSeparator />
      ...
      <div className="flex gap-4">
        ...
        {showImagePanel && suggestImagesMutation.isPending && (
          <ScanPanelSkeleton
            title="Add Images"
            icon={<ImagePlus className="w-4 h-4 text-purple-600" />}
            gradientFrom="from-purple-50"
            gradientTo="to-fuchsia-50"
            accentColor="text-purple-600"
            description="Analyzing article sections to suggest optimal image placements"
            onClose={() => setShowImagePanel(false)}
            onCancel={() => { suggestImagesMutation.reset(); setShowImagePanel(false); }}
          />
        )}
        {showImagePanel && !suggestImagesMutation.isPending && (
          <ImagePanel
            suggestions={imageSuggestions}
            existingImages={extractExistingImages(editor?.getHTML() || "")}
            onClose={() => setShowImagePanel(false)}
            onGenerate={async (suggestion, idx) => {
              setGeneratingImageIdx(idx);
              try {
                const result = await generateSingleImageMutation.mutateAsync({
                  prompt: suggestion.prompt,
                  altText: suggestion.altText,
                });
                if (result.url && editor) {
                  // Insert the image after the matching heading
                  const html = editor.getHTML();
                  const headingEscaped = suggestion.afterHeading.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
                  const headingRegex = new RegExp(
                    `(<h[23][^>]*>[^<]*${headingEscaped}[^<]*<\\/h[23]>)`,
                    "i"
                  );
                  const figureHtml = `<figure class="ai-generated-image" data-prompt="${suggestion.prompt.replace(/"/g, "&quot;")}" style="margin: 24px 0; text-align: center;"><img src="${result.url}" alt="${suggestion.altText.replace(/"/g, "&quot;")}" style="max-width: 100%; height: auto; border-radius: 8px;" /><figcaption style="font-size: 0.85em; color: #666; margin-top: 8px;">${suggestion.altText}</figcaption></figure>`;
                  if (headingRegex.test(html)) {
                    editor.commands.setContent(html.replace(headingRegex, `$1${figureHtml}`));
            ...
            onRegenerate={(imageUrl: string, prompt: string) => {
            ...
            onRemove={(imageUrl: string) => {
              removeImageMutation.mutate({ articleId, imageUrl });
            }}
            isGenerating={generatingImageIdx}
...
/* ── Helper: Extract existing images from article HTML ── */
function extractExistingImages(html: string): Array<{ url: string; altText: string; prompt: string }> {
  const images: Array<{ url: string; altText: string; prompt: string }> = [];
  const figureRegex = /<figure[^>]*class="ai-generated-image"[^>]*data-prompt="([^"]*)"[^>]*>[\s\S]*?<img[^>]*src="([^"]*)"[^>]*alt="([^"]*)"[\s\S]*?<\/figure>/gi;
  let match: RegExpExecArray | null;
  while ((match = figureRegex.exec(html)) !== null) {
    images.push({
      prompt: match[1].replace(/&quot;/g, '"'),
      url: match[2],
      altText: match[3].replace(/&quot;/g, '"'),
    });
  }
  return images;
}

/* ── Image Management Panel Component ── */
type ImageSuggestion = { afterHeading: string; prompt: string; altText: string };
type ExistingImage = { url: string; altText: string; prompt: string };
...
function ImagePanel({
  ...
  onClose: () => void;
  onGenerate: (suggestion: ImageSuggestion, idx: number) => void;
  onRegenerate: (imageUrl: string, prompt: string) => void;
  ...
  return (
    <div className="w-80 bg-white rounded-xl border border-border/60 p-5 space-y-4 flex-shrink-0 self-start sticky top-4 max-h-[calc(100vh-120px)] overflow-y-auto">
      ...
      {existingImages.length > 0 && (
        <div className="space-y-2">
          ...
          {existingImages.map((img, i) => (
            <div key={i} className="border border-border/40 rounded-lg p-2.5 space-y-2 bg-gray-50/50">
              <img
                src={img.url}
                alt={img.altText}
                className="w-full h-24 object-cover rounded-md"
              />
              <p className="text-xs text-muted-foreground line-clamp-2">{img.altText}</p>
              <div className="flex gap-1.5">
                <Button
                  ...
                  onClick={() => {
                    setRegeneratingUrl(img.url);
                    setRegeneratePrompt(img.prompt);
                  }}
                  disabled={isRegenerating || isRemoving}
                >
                ...
                <div className="space-y-1.5 pt-1">
                  ...
                  <div className="flex gap-1.5">
                    <Button
                      ...
                      onClick={() => {
                        onRegenerate(img.url, regeneratePrompt);
                    ...
                    <Button
                      ...
                      onClick={() => setRegeneratingUrl(null)}
                    >
                      Cancel
                    </Button>
                  </div>
      ...
      {suggestions.length > 0 && (
        <div className="space-y-2">
          <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
            Suggested Placements ({suggestions.length})
          </p>
          {suggestions.map((suggestion, i) => (
            <div key={i} className="border border-purple-100 rounded-lg p-2.5 space-y-2 bg-purple-50/30">
              <div className="flex items-start gap-2">
                <Image className="w-3.5 h-3.5 text-purple-500 mt-0.5 flex-shrink-0" />
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-medium text-foreground line-clamp-1">
                    After: {suggestion.afterHeading}
                  </p>
                  {editingPromptIdx === i ? (
                    <div className="space-y-1.5 mt-1">
                      ...
                      <div className="flex gap-1.5">
                        <Button
                          ...
                          className="flex-1 text-xs h-7 bg-purple-600 hover:bg-purple-700"
                          onClick={() => {
                            onGenerate({ ...suggestion, prompt: editedPrompt }, i);
                            setEditingPromptIdx(null);
                          }}
                          disabled={isGenerating !== null}
                        >
                          {isGenerating === i ? <Loader2 className="w-3 h-3 mr-1 animate-spin" /> : <Sparkles className="w-3 h-3 mr-1" />}
                          Generate
                        </Button>
                        <Button
                          variant="outline"
              ...
              {editingPromptIdx !== i && (
                <div className="flex gap-1.5">
                  <Button
                    size="sm"
                    className="flex-1 text-xs h-7 bg-purple-600 hover:bg-purple-700"
                    onClick={() => onGenerate(suggestion, i)}
                    disabled={isGenerating !== null}
                  >
                    {isGenerating === i ? (
                      <>
                  ...
                  <Button
                    ...
                    onClick={() => {
                      ...
                      setEditedPrompt(suggestion.prompt);
                    }}
                    disabled={isGenerating !== null}
                  >
      ...
      {/* Empty state */}
      {suggestions.length === 0 && existingImages.length === 0 && (
        <div className="text-center py-6">
          ...
          <p className="text-xs text-muted-foreground mt-1">
            No image placement suggestions were found for this article
          </p>
        </div>
      )}
    </div>
